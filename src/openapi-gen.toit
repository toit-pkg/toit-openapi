...
For now this is a placeholder: every non-primitive schema becomes a
  $toit-gen.Class.imported stub with the schema's preferred name. Once
  toit-json-schema's `Gen` exposes a "populate into existing library" entry
  point with a public `ClassManager`, this resolver should delegate to it so
  model classes generated by json-schema can be referenced directly from the
...
class TypeResolver:
  ...
  /// One $toit-gen.Class per unique json-schema $Schema, keyed by identity.
  resolved/Map ::= {:}

  /**
  Returns the $toit-gen.Class to use as the type for $open-api-schema.

  $hint is used as a fallback name when the schema has no nameable URL.
  */
  resolve open-api-schema/Schema? --hint/string -> toit-gen.Class:
    if not open-api-schema: return any-class
...
class ApiGenerator:
  ...
  */
  build-operation_ tag-class/toit-gen.Class
    ...
    request-body-type/toit-gen.Class? := null
    ...
    if operation.request-body:
      ...
      request-body-type = content.contains "application/json"
          ? type-resolver.resolve content["application/json"].schema --hint="$path-$(method)-request-body"
          : type-resolver.byte-array-class
    ...
    // --- Regular variant ---
    // op-name [--p1=...] [--p2=...] [body/Body]:
    //   op-name --raw [--p1=p1] [--p2=p2] [body]
    //   return null
    ...
    regular-fn := tag-class.add-method op-name --parameters=regular-fn-params regular-body
  ...
  */
  ...
  body-vd_ type/toit-gen.Class -> toit-gen.VarDefinition:
    return toit-gen.VarDefinition.parameter "body" --type=(toit-gen.Ref type)
